...
def broken_rows(conn: sqlite3.Connection) -> list[sqlite3.Row]:
    ...
    rows = conn.execute(
        "SELECT item_key, address, lat, lng, geocoded_at FROM auction_items "
        "WHERE coordinate_source='building' AND is_active=1 AND lat IS NOT NULL"
    ).fetchall()
    return [r for r in rows if _extract_building_hint(r["address"] or "") in SIDO]
...
def selfcheck() -> None:
    ...
    assert _extract_building_hint(land) in SIDO, _extract_building_hint(land)
    ...
    print("자가 점검 통과: 시도명 힌트만 가짜로 잡고, 진짜 건물명은 안 건드린다")
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    selfcheck() if a.check else main(a.limit, a.seed)

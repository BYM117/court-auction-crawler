...
def checks() -> list[dict]:
    ...
    def add(no, title, owner, state, measure, note=""):
        out.append({"no": no, "title": title, "owner": owner,
                    "state": state, "measure": measure, "note": note})

    # G01 — 사건 화면으로 낙찰가 회수
    sold = q1("SELECT COUNT(*) FROM auction_sale_results WHERE sale_amount IS NOT NULL")
    add("G01", "낙찰가 회수(사건 화면)", "A",
        DONE if src_has("detail_crawler.py", "기일내역에서 매각결과") else TODO,
        f"낙찰가 {sold:,}건" if sold is not None else "—",
        "보충이 도는 중이면 계속 늘어난다")

    # G02 — 지분 딱지
    share = src_has("enrichment.py", "지분매각")
    add("G02", "지분매각 딱지", "B", DONE if share else TODO,
        "special_rights에 포함" if share else "없음")
    ...
    add("G05", "되살아난 물건의 낙찰가", "B",
        DONE if (stale or 0) <= 5 else TODO,
        f"활성인데 낙찰가 박힘 {stale:,}건", "0에 가까워야 정상")
    ...
    # G08 — 인근매각통계
    near = src_has("detail_crawler.py", "인근매각통계")
    add("G08", "인근매각통계 채우기", "A", DONE if near else TODO,
        "수집 코드 있음" if near else "빈 표 그대로")

    # G09 — 입찰구분
    bid = src_has("crawler.py", "BidLst")
    add("G09", "입찰구분을 '전체'로", "A", DONE if bid else TODO,
        "설정함" if bid else "기일입찰 기본값 그대로", "지금 손실 0, 재개 대비")
    ...
    add("G12", "엉뚱한 좌표", "B",
        DONE if (broken is not None and broken < 10) else TODO,
        f"망가진 쿼리 {broken}건 · approximate {approx:,}건" if broken is not None else "—",
        "망가진 쿼리 10건 미만이면 해결로 본다")
    ...
    # G14 — 위험도
    gain = src_has("enrichment.py", "score +=")
    add("G14", "위험도 재설계", "E",
        DONE if gain else TODO,
        "가점 경로 있음" if gain else "감점만 있어 '낮음' 도달 불가",
    ...
    add("G16", "문서 조회 창(제약)", "전체", FIXED_LIMIT,
        "명세서 기일 1주 전~ / 조사서·평가서 2주 전~", "고칠 것 없음. 알고만 있을 것")
...
def render(rows: list[dict]) -> str:
    ...
    for s in ORDER:
        for r in sorted((x for x in rows if x["state"] == s), key=lambda x: x["no"]):
            ...
            L.append(f"| {ICON[s]} | {link} | {r['title']} | {r['owner']} | {r['measure']} |")
    L.append("")

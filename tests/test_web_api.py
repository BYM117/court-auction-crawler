...
class WebApiTests(unittest.TestCase):
    ...
    def test_detail_endpoint_returns_public_shape(self):
        ...
        # 무조건 붙던 '권리확인 필요' 잡음은 사라졌다. 옛 키는 지키고 reasons 를 더했다.
        self.assertEqual(payload["screening"]["risk_level"], "낮음")
        self.assertEqual(payload["screening"]["flags"], [])
        self.assertEqual(payload["screening"]["reasons"], [])
        self.assertEqual(set(payload["screening"]), {"score", "risk_level", "flags", "reasons"})

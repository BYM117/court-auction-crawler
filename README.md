...
상세 수집 데몬과 같은 락(`data/collect-details.pid`)을 쓰므로 한 번에 하나만 돕니다. 데몬을 내리고 정해진 시간만큼 패스를 반복한 뒤 데몬을 되돌리는 일은 스크립트가 합니다.

```bash
scripts/run_backfill_results.sh 430    # 430분 동안 돌고 데몬 복구
```

사이트가 차단하면 수집기가 스스로 패스를 접는데, 냉각 후 새 브라우저로 이어갑니다. 평소 상세 수집도 사건 화면에 들른 김에 빠진 결과행을 함께 메웁니다.

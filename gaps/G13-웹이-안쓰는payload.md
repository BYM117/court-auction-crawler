...
## 2026-09-18 운영 실측 — '38%' 와 '17%' 는 분모가 다르다

고친 코드가 실제로 만든 데이터(`match_level` 이 있는 609건)를 종류별로 갈라 봤다.

| | 건수 | parcel | name | **이 물건의 값** |
|---|---|---|---|---|
| 건물(아파트·빌라·오피스텔) | 293 | 20% | 16% | **37%** |
| **토지** | 316 | 0% | 0% | **0%** |
| 전체 | 609 | 9% | 8% | 17% |

표본 측정(38%)은 `apart/villa/officetel` 만 걸렀고, 운영에는 **토지가 절반**이다.
**둘 다 맞는 숫자이고 분모가 다르다.** 건물만 보면 37% 로 표본과 사실상 같다.

### 토지 0% 는 버그가 아니다

토지 실거래 API(`RTMSDataSvcLandTrade`)도 `umdNm`·`jibun` 을 준다. 필지 대조는
제대로 걸린다. 다만 **특정 지번이 최근 6개월에 거래될 확률이 낮다** — 강동구
전체 토지 거래가 한 달 15건이다. `dong`(13%)·`sigungu`(86%) 로는 잡히므로
동네 시세는 나온다. 이건 데이터의 성질이지 코드의 문제가 아니다.
